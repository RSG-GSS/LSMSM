...
##########################################################################   
##### 10/12/2017: Modified.
##### 10/11/2017: Previously modified.
##### 09/29/2017: Created.
##### Description: 
...
#####		10/10/2017:
#####			- Change objectivefunc2!() to objectivefunc2().
#####			- Insert assertion logic to check result.
#####		10/11/2017:
#####			- Begin defining parallel function.
#####		10/12/2017:
#####			- Remove @everywhere from getopt2wrapper().
##########################################################################
...
##### 9/29/17 B1HXM10: Wrapper for getopt2().
...
##### 9/29/17 B1HXM10: Function for implementing wrapper.
function objectivefunc2(p::Array{Float64,1},fp::fparams,moments::structureM)
    sd = 2230
    srand(sd)  
    p0 = pv2p0(p)
    gr = grids(fp,p0)    
    ...
    for rep = 1:fp.nsim
				...
				sim[:,rep]=[getopt2wrapper(sim,rep,i,a,rvw,rvu,lEDU,lEV,fp,p0,gr) for i = 1:fp.nind]
...
##### 10/10/17 B1HXM10: Function for implementing parallel processing.
function objectivefunc2p(p::Array{Float64,1},fp::fparams,moments::structureM)
    sd = 2230
    srand(sd)  
    p0 = pv2p0(p)
    gr = grids(fp,p0)    
